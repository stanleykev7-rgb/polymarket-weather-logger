...
# Explicitly maintain strict 8-column CSV schema
CSV_COLUMNS = [
    "timestamp_utc",
    "city",
    "target_date",
...
def format_buckets_readable(bucket_dict):
  ...
  for bucket, price in bucket_dict.items():
    pct = price * 100
    if pct >= 0.5:  # Filter out noise (< 0.5%)
...
def log_snapshot():
  ...
  for city_name, info in CITIES.items():
    try:
      ...
      # CSV row: Strictly 8 fields matching CSV_COLUMNS
      records.append({
          ...
          "all_bucket_prices": json.dumps(
              poly_prices, ensure_ascii=False
          ),  # Raw JSON string
      })

      # Console Print (Uses the readable string format safely outside the CSV)
      print(f"[{city_name} | {target_date}]")
      print(
          f"  Forecasts: ECMWF = {ecmwf_t}°C (Bucket: {predicted_bucket}) | GFS"
          f" = {gfs_t}°C"
      )
      print(
          "  Poly Price"
          f" ({predicted_bucket}): {matching_price if matching_price is not None else 'N/A'}"
      )
  ...
  if records:
    ...
    # Append safely while enforcing column structure
    df.to_csv(
        CSV_FILE,
        mode="a",
